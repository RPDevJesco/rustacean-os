...
pub struct Window {
    ...
    /// Content buffer (what the application draws to)
...
impl Window {
    ...
    /// Draw text in the content area
    pub fn draw_text(&self, fb: &mut Framebuffer, x: i32, y: i32, text: &str) {
        let theme = theme::current();
        let abs_x = self.bounds.x + BORDER_WIDTH as i32 + x;
        let abs_y = self.bounds.y + TITLE_HEIGHT as i32 + y;
        
        fb.draw_string(abs_x, abs_y, text, theme.text, Some(theme.window_bg));
    }
